...
# 🔐 Cluster 1 – Flyway Grants & Access Control (Aurora PostgreSQL)

## Overview
This repository manages all **role, privilege, and access control migrations** for **Cluster 1**,  
which runs on **Amazon Aurora PostgreSQL**. It complements the `flyway-1-pipeline` repository  
...
- Keep schema-altering DDL out of this repo — that belongs to `flyway-1-pipeline`.
...
## 🔄 Deployment Order
1. Apply schema migrations from `flyway-1-pipeline`
2. Run grant and role migrations from `flyway-1-grants`
...
- Include environment identifiers (e.g., `cluster1`, `aurora_pg`) in migration comments.
